...
def route(messages, brain="auto", max_tokens=512):
    if brain == "local":
        try:
            return {"reply": call_local(messages, max_tokens), "brain": "local", "ok": True}
        except Exception as e:
            return {"reply": "", "brain": "local", "ok": False, "error": str(e)}
    if brain == "gemini":
        try:
            return {"reply": call_gemini(messages, max_tokens), "brain": "gemini", "ok": True}
        except Exception as e:
    ...
    try:
        ...
        try:
            return {"reply": call_local(messages, max_tokens), "brain": "local", "ok": True, "fallback": True}
        except Exception as e:
            return {"reply": "", "brain": "none", "ok": False, "error": str(e)}
